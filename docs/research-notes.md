...
## ۱۲) باگ «گیج‌زدن» در گوگل‌ترنسلیت و ChatGPT — میزبانی که متن ما را بازنویسی می‌کند

گزارش کاربر: «یکم داره گیج میزنه، مخصوصاً در چت‌جی‌پی‌تی و ترنسلیت گوگل». با یک harness در مرورگر
**بازتولید شد** (نه حدس): یک `<textarea>` که روی هر رویداد `input` محتوا را بازنویسی می‌کند.

```
setInterim('ab') → setInterim('abc') → setInterim('abcd')
انتظار: "ABCD"        نتیجهٔ واقعی: "ABABCABCD"   ← تکرار افسارگسیخته
```

**علت ریشه‌ای**: `FieldWriter` دنبالهٔ interim را با تطبیق متن در anchor «مال خودش» می‌شناسد. ویرایشگرهایی
مثل گوگل‌ترنسلیت (که ترنسلیتریشن خودش را می‌نویسد) و ProseMirror/ChatGPT (که محتوا را نرمال‌سازی می‌کند)
متن ما را عوض می‌کنند؛ دفعهٔ بعد دنباله پیدا نمی‌شود، پس به‌جای **جایگزینی**، interimِ روبه‌رشد دوباره
**الحاق** می‌شود و متن می‌ترکد.

نکتهٔ ظریف: میزبانی که فقط **مکان‌نما** را جابه‌جا می‌کند مشکلی ایجاد نمی‌کرد (تست شد و پاس شد) — فقط
**بازنویسی محتوا** خراب‌کننده است.

**رفع**: بعد از هر نوشتنِ interim، همان‌جا readback می‌کنیم. اگر میزبان چیزی را که نوشتیم نگه نداشته
بود، قطعهٔ خودمان را برمی‌داریم و برای آن فیلد **interim زنده را خاموش می‌کنیم** و فقط finalها را commit
می‌کنیم — یعنی همان رفتار امنِ قبل از ۰.۱.۱ روی میزبان‌های سرکش، به‌جای متنِ به‌هم‌ریخته. commitها
دست‌نخورده کار می‌کنند (میزبان آن‌ها را هم تبدیل می‌کند، ولی آن دیگر متنِ کاربر است).

## ۱۳) اندازه‌گیری قطعی: چرا «گیر می‌کرد» — و اینکه دو سایت دو مشکل جدا داشتند

با افزودن آمار سمت content script، دو جلسهٔ واقعی روی همان متن:

```
translate.google.com  kind: textarea         interimCalls: 129  interimWrites: 79  commits: 2  reanchors: 0  liveInterim: true
chatgpt.com           kind: contenteditable  interimCalls: 137  interimWrites:  0  commits: 7  reanchors: 0  liveInterim: false
```

این اعداد سه چیز را **ثابت** کردند:

1. **گوگل‌ترنسلیت سالم بود.** `reanchors: 0` و `liveInterim: true` یعنی مالکیت دنباله هیچ‌وقت از دست نرفت و
   نوشتن زنده کار می‌کرد. علاوه بر این، کدِ واقعی نویسنده را روی **خودِ صفحهٔ ترنسلیت** اجرا کردیم: متن دقیقاً
   درست درآمد، میزبان نه محتوا را بازنویسی می‌کرد نه مکان‌نما را جابه‌جا، و هر نوشتن ~۲.۴ms طول می‌کشید.
   یعنی فرضیهٔ «میزبانِ بازنویسی‌کننده» برای ترنسلیت **غلط** بود.
2. **ChatGPT یک مشکل کاملاً متفاوت داشت**: `contenteditable` است و نویسندهٔ ما برای این نوع فیلد عمداً
   interim را خاموش می‌کرد → `interimWrites: 0`. متن فقط در ۷ نوبت (پایان هر segment) ظاهر می‌شد و همین
   حسِ «گیر کردن» را می‌داد. **باگ نبود، طراحی بود** — ولی طراحی بدی بود.
3. **کم‌بودن `commits`** (۲ و ۷ در یک دیکتهٔ طولانی) یعنی کروم segmentهای بسیار بلندی نگه می‌دارد. متنی که
   وسطِ یک segment بلند گم می‌شود، اصلاً به ما نمی‌رسد؛ این محدودیت موتور است نه خط لولهٔ ما.

**رفع مورد ۲**: ویرایشگرهای غنی (ProseMirror/ChatGPT، Gmail) مدل سند خودشان را دارند و دست‌کاری مستقیم
DOM را برمی‌گردانند؛ ولی **انتخاب واقعی + `execCommand('insertText')`** را می‌فهمند. پس دنبالهٔ interim را
با `selection.modify('extend','backward','character')` انتخاب می‌کنیم و **فقط بعد از تأیید اینکه محتوای
انتخاب‌شده دقیقاً دنبالهٔ خودمان است** بازنویسی می‌کنیم؛ در غیر این صورت انتخاب را جمع می‌کنیم و interim
زنده را برای آن فیلد خاموش می‌کنیم. هرگز چیزی را که تأیید نکرده‌ایم حذف نمی‌کنیم.

نکتهٔ ظریفی که تست گرفت: contenteditable فاصلهٔ کنار مکان‌نما را به `U+00A0` تبدیل می‌کند، پس مقایسهٔ خام
رشته‌ها روی اولین دنبالهٔ دارای فاصله شکست می‌خورد و interim را برای همیشه خاموش می‌کرد. مقایسه حالا نسبت
به nbsp بی‌تفاوت است.

## ۱۴) «فقط فاصله می‌ذاشت به ازای هر جمله» — finalهای خالی

گزارش کاربر روی ChatGPT: اول کلمه‌به‌کلمه می‌نوشت، بعد جمله‌به‌جمله شد، و در آخر **فقط یک فاصله** به ازای
هر جمله می‌گذاشت. آمار همان جلسه: `interimWrites: 45` (پس اول واقعاً زنده کار می‌کرد) و `liveInterim: false`
(پس وسط کار به fallback افتاد).

علتِ فاصله‌ها با اجرای مستقیم accumulator پیدا شد — حدس نبود:

```
blank final   -> {finalText: " ",    interimText: ""}
spaces final  -> {finalText: "    ", interimText: ""}
normal final  -> {finalText: "hello ", interimText: ""}
```

کروم در جلسه‌های طولانی **finalهای خالی** هم می‌فرستد، و ما برای هرکدام `'' + ' '` را commit می‌کردیم؛
یعنی وفادارانه یک فاصله در فیلد کاربر تایپ می‌کردیم. حالا finalهایی که بعد از نرمال‌سازی خالی‌اند نادیده
گرفته می‌شوند (ولی شمارنده جلو می‌رود تا دوباره پردازش نشوند).

## ۱۵) تصمیم: در ویرایشگرهای غنی، interim زنده را رها کردیم

سه بار برای ProseMirror/ChatGPT تلاش شد و هر بار به شکل تازه‌ای شکست: (۱) اصلاً ننوشتن، (۲) انتخابِ
تأییدنشده، (۳) تأیید nbsp-محور که باز هم وسط کار از کار افتاد و **قطعهٔ یتیم** وسط جملهٔ کاربر جا گذاشت.

الگو روشن است: ویرایشگری که مدل سند خودش را دارد، دیر یا زود متن ما را بازچینی می‌کند و آن‌وقت دنباله
نه قابل تأیید است نه قابل حذف. **نوشتن کلمه‌به‌کلمه یک خوش‌آیندی است؛ خراب‌کردن پیامی که کاربر می‌خواهد
بفرستد نیست.** پس ویرایشگرهای غنی فقط متنِ نهایی می‌گیرند (عبارت‌به‌عبارت) و فیلدهای ساده
(`input`/`textarea`) کلمه‌به‌کلمه. هیچ متنی گم نمی‌شود؛ فقط دیرتر می‌رسد.

## ۱۶) گم‌شدنِ جمله‌های کامل — بالادستِ ما، و راه کاهشش

بعد از رفع فاصله‌ها، هنوز دو جملهٔ کامل از یک دیکتهٔ چندجمله‌ای گم شد. آمار همان جلسه:
`commits: 3` برای چهار جمله. یعنی کروم یک segment را خیلی طولانی باز نگه داشته بود.

مکانیزم: تا وقتی segment نهایی نشده، موتور آزاد است متن را بازنگری کند و بخشی از آنچه در interim
نشان داده بود را **دور بریزد**. آن متن اصلاً به ما نمی‌رسد که بنویسیمش — پس نه writer مقصر است نه
accumulator. شاهد جانبی: در همان آزمایش‌ها هرجا `commits` بیشتر بود (segmentهای کوتاه‌تر) متن
کامل‌تر درمی‌آمد.

**کاهش (نه رفع کامل)**: segmentها را کوتاه نگه می‌داریم. اگر یک segment بیش از ۸ ثانیه باز مانده باشد،
آن را می‌بندیم تا موتور مجبور شود همان‌جا commit کند. اما **فقط وقتی میکروفون واقعاً ساکت است**
(سطح صدا زیر آستانه برای ۷۰۰ms) — چون `stop()` وسط کلمه، در فاصلهٔ چند صد میلی‌ثانیه‌ایِ restart صدا را
می‌بُرد و یک «افت گاه‌به‌گاه» را به یک «افت قطعی» تبدیل می‌کرد. نشانگر سطح صدا از قبل وجود داشت و
همان را به‌کار گرفتیم.

برای اینکه بعداً هم بشود قضاوت کرد، خلاصهٔ جلسه دو عدد تازه دارد:
- `forcedSegments`: چند بار در سکوت segment را بستیم.
- `droppedChars`: مجموع کاراکترهایی که interim نشان داده بود ولی final نگه نداشت — معیار مستقیم
  افتِ سمتِ موتور. اگر این عدد بزرگ بماند، مشکل بالادستی است و از دست ما خارج.

## ۱۷) تصحیح § ۱د — `host_permissions` هیچ‌وقت لازم نبود

بخش ۱د نتیجه گرفته بود که چون `runtime.sendMessage` به content script نمی‌رسد و باید از
`tabs.sendMessage` استفاده کرد، پس `host_permissions: ['<all_urls>']` لازم است. **نیمهٔ اول درست
بود، نیمهٔ دوم نه.** آن مجوز هم‌زمان با راه‌حل واقعی اضافه شده بود و کسی جدا آزمایشش نکرده بود.

مستندات کروم هیچ‌جا نمی‌گویند `tabs.sendMessage` به host permission نیاز دارد. یک بیلد آزمایشی
بدون آن ساخته شد و **دیکته کاملاً کار کرد** (یوتیوب، جلسه‌ای با `errors: []` و `droppedChars: 0`).
منطقش هم روشن است: content script از طریق `content_scripts.matches` تزریق می‌شود و همان خودش
مجوزِ حضور است؛ پیام‌دادن به اسکریپتِ خودمان مجوز جداگانه نمی‌خواهد.

تنها جایی که واقعاً دسترسی می‌خواست، خواندن `tab.url` در پاپ‌آپ تولبار برای کلید پر-سایت بود.
با **`activeTab`** پوشش داده شد: دقیقاً موقع صدازدنِ افزونه توسط کاربر (یعنی همان لحظهٔ بازشدن
پاپ‌آپ) داده می‌شود و هشدار نصب اضافه نمی‌کند. مجوز `tabs` عمداً انتخاب **نشد** چون هشدارش
(«خواندن تاریخچهٔ مرور») از host_permissions هم بدتر است.

### اندازه‌گیریِ آنچه واقعاً عوض شد (مهم، تا انتظار اشتباه نسازیم)

بعد از حذف، صفحهٔ Details کروم همچنان می‌گوید **Site access: On all sites**، و بخش Permissions
خالی است (`offscreen`/`storage`/`activeTab` هیچ‌کدام هشدار کاربر تولید نمی‌کنند).

یعنی: **هشدار نصبِ کاربر عوض نشد** — آن را `content_scripts.matches: ['<all_urls>']` تولید می‌کند
که ذاتیِ محصول است و نمی‌رود. سودِ واقعی این تغییر:
- پرچمِ «Broad Host Permissions» در ارسال به فروشگاه برداشته می‌شود،
- و سطح دسترسیِ واقعی کوچک‌تر می‌شود (که فارغ از هشدار، از نظر امنیتی درست است).
